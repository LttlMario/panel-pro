(() => {
  ...
  const renderPreview = () => {
    ...
    const descriptions = { full: 'Toate modulele Panel Pro cu butoane, plus canalele lor de rezultate și loguri: legale, ilegale, management, Stash, pontaj, rapoarte și Status Live.', legal_management: 'Modulele legale și de management cu butoane, împreună cu logurile pentru învoiri, pontaj, contracte, marketplace și disciplină.', illegal: 'Modulele ilegale și operaționale cu butoane, împreună cu logurile pentru marketplace, disciplină, acțiuni și Stash.' };
    preview.innerHTML = `<div class="flex items-center justify-between gap-2"><strong>${esc(bundle.label)}</strong><span class="tag">rute reale Panel Pro</span></div><p class="muted mt-2">${esc(descriptions[select.value] || bundle.description)}</p><div class="grid gap-2 sm:grid-cols-2 mt-3"><div class="rounded-lg border border-slate-800 bg-slate-950/60 p-2"><strong>Embeduri și butoane reale</strong><br><small class="muted">Definițiile sunt cele folosite de botul Panel Pro.</small></div><div class="rounded-lg border border-slate-800 bg-slate-950/60 p-2"><strong>Canale și loguri</strong><br><small class="muted">Rutele sunt salvate automat în Administrare organizații.</small></div><div class="rounded-lg border border-slate-800 bg-slate-950/60 p-2"><strong>Rapoarte automate</strong><br><small class="muted">Săptămânale, export contracte și remindere.</small></div><div class="rounded-lg border border-slate-800 bg-slate-950/60 p-2"><strong>Status Live</strong><br><small class="muted">Canal configurat pentru sincronizarea live.</small></div></div><p class="muted text-xs mt-3">Nu se creează roluri noi. Botul primește acces la canale, iar configurația este folosită atât de web, cât și de bot.</p>`;
  };
  ...
  const installBundle = async () => {
    ...
    try {
      ...
      setStatus(`Pachet instalat: ${result.created?.channels || 0} canale și ${result.created?.messages || 0} embeduri publicate. Fiecare modul are logul lui; nu au fost create roluri noi.`);
