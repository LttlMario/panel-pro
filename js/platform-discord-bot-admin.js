(() => {
  ...
  const render = () => {
    ...
    $('list').querySelectorAll('[data-delete-org]').forEach((button) => button.addEventListener('click', () => deleteDiscordOrganization(button.dataset.deleteOrg)));
  };
  ...
  const deleteDiscordOrganization = async (organizationId) => {
    const organization = state.organizations.find((item) => String(item.id) === String(organizationId));
    if (!organization) return;
    const name = String(organization.name || '').trim();
    if (!window.confirm(`Ștergi definitiv serverul Discord-only „${name}”? Vor fi șterse organizația și datele ei asociate. Această acțiune nu poate fi anulată.`)) return;
    const confirmation = window.prompt(`Pentru confirmare, scrie exact numele serverului/organizației:\n${name}`);
    if (confirmation === null) return;
    if (confirmation.trim() !== name) { status('Ștergerea a fost anulată: numele introdus nu corespunde.', 'error'); return; }
    try { await api({ action: 'delete', organization_id: organizationId, confirm_name: confirmation.trim() }); status('Serverul Discord-only și datele asociate au fost șterse.', 'ok'); await load(); } catch (error) { status(error.message || 'Serverul Discord-only nu a putut fi șters.', 'error'); }
  };
