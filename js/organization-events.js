(() => {
  ...
  function render() {
    ...
    $('events-list').innerHTML = events.length ? events.map((event) => {
      ...
      const actions = canWrite ? `<div class="flex flex-wrap gap-2"><button type="button" data-edit="${esc(event.id)}" class="rounded-lg border border-slate-700 px-3 py-2 text-[11px] font-bold text-slate-300 hover:bg-slate-800">Editează</button>${current.key !== 'archived' ? `<button type="button" data-archive="${esc(event.id)}" class="rounded-lg border border-amber-800/70 px-3 py-2 text-[11px] font-bold text-amber-200 hover:bg-amber-950/50">Arhivează</button><button type="button" data-send-now="${esc(event.id)}" class="rounded-lg border border-emerald-700/70 px-3 py-2 text-[11px] font-bold text-emerald-200 hover:bg-emerald-950/50">Trimite reminder acum</button>` : `<button type="button" data-unarchive="${esc(event.id)}" class="rounded-lg border border-emerald-700/70 px-3 py-2 text-[11px] font-bold text-emerald-200 hover:bg-emerald-950/50">Reactivează</button>`}<button type="button" data-delete="${esc(event.id)}" class="rounded-lg border border-rose-800/70 px-3 py-2 text-[11px] font-bold text-rose-200 hover:bg-rose-950/50">Șterge</button><button type="button" data-test="${esc(event.id)}" class="rounded-lg border border-cyan-700/70 px-3 py-2 text-[11px] font-bold text-cyan-200 hover:bg-cyan-950/50">${localMode ? 'Simulează reminderul' : 'Testează postarea botului'}</button></div>` : '';
  ...
  $('events-list').addEventListener('click', async (event) => {
    const editId = event.target.closest('[data-edit]')?.dataset.edit; const archiveId = event.target.closest('[data-archive]')?.dataset.archive; const unarchiveId = event.target.closest('[data-unarchive]')?.dataset.unarchive; const sendNowId = event.target.closest('[data-send-now]')?.dataset.sendNow; const deleteId = event.target.closest('[data-delete]')?.dataset.delete; const testId = event.target.closest('[data-test]')?.dataset.test;
    ...
    if (unarchiveId) { if (!confirm('Reactivezi evenimentul și reminderele lui?')) return; try { if (localMode) { events = events.map((item) => String(item.id) === String(unarchiveId) ? { ...item, status: 'active', archived_at: null } : item); saveLocalEvents(events); } else { const result = await api('unarchive', { id: unarchiveId }); events = events.map((item) => String(item.id) === String(unarchiveId) ? result.event : item); } render(); } catch (error) { alert(error.message || 'Reactivarea a eșuat.'); } return; }
    if (sendNowId) { if (!confirm('Trimiți reminderul de astăzi acum în logul Discord?')) return; try { if (localMode) { alert('Reminder simulat local. Nu s-a trimis nimic pe Discord.'); } else { const result = await api('send_now', { id: sendNowId }); alert(result.notification?.status === 'sent' ? 'Reminderul a fost trimis în logul Discord.' : result.notification?.status === 'already_sent' ? 'Reminderul de astăzi a fost deja trimis.' : `Reminderul nu a fost trimis: ${result.notification?.error || 'destinația nu este configurată.'}`); const refreshed = await api('list'); events = refreshed.events || events; reminders = refreshed.reminders || reminders; render(); } } catch (error) { alert(error.message || 'Reminderul nu a putut fi trimis.'); } return; }
