(() => {
  ...
  function render() {
    ...
    $('events-list').innerHTML = events.length ? events.map((event) => {
      ...
      const actions = canWrite ? `<div class="flex flex-wrap gap-2"><button type="button" data-edit="${esc(event.id)}" class="rounded-lg border border-slate-700 px-3 py-2 text-[11px] font-bold text-slate-300 hover:bg-slate-800">Editează</button>${current.key !== 'archived' ? `<button type="button" data-archive="${esc(event.id)}" class="rounded-lg border border-amber-800/70 px-3 py-2 text-[11px] font-bold text-amber-200 hover:bg-amber-950/50">Arhivează</button>` : `<button type="button" data-unarchive="${esc(event.id)}" class="rounded-lg border border-emerald-700/70 px-3 py-2 text-[11px] font-bold text-emerald-200 hover:bg-emerald-950/50">Reactivează</button>`}<button type="button" data-delete="${esc(event.id)}" class="rounded-lg border border-rose-800/70 px-3 py-2 text-[11px] font-bold text-rose-200 hover:bg-rose-950/50">Șterge</button><button type="button" data-test="${esc(event.id)}" class="rounded-lg border border-cyan-700/70 px-3 py-2 text-[11px] font-bold text-cyan-200 hover:bg-cyan-950/50">${localMode ? 'Simulează reminderul' : 'Testează postarea botului'}</button></div>` : '';
