(() => {
  ...
  const EVENT_TYPES = Object.freeze({ car_meet: 'Car Meet', convoy: 'Convoy', race: 'Cursă / Race', party: 'Petrecere IC', community: 'Eveniment comunitar', roleplay: 'Eveniment RP / IC', other: 'Alt eveniment IC' });
  ...
  function render() {
    ...
    $('events-list').innerHTML = events.length ? events.map((event) => {
      ...
      const actions = canWrite ? `<div class="flex flex-wrap gap-2"><button type="button" data-edit="${esc(event.id)}" class="rounded-lg border border-slate-700 px-3 py-2 text-[11px] font-bold text-slate-300 hover:bg-slate-800">Editează</button>${current.key !== 'archived' ? `<button type="button" data-archive="${esc(event.id)}" class="rounded-lg border border-amber-800/70 px-3 py-2 text-[11px] font-bold text-amber-200 hover:bg-amber-950/50">Arhivează</button>` : ''}<button type="button" data-test="${esc(event.id)}" class="rounded-lg border border-cyan-700/70 px-3 py-2 text-[11px] font-bold text-cyan-200 hover:bg-cyan-950/50">${localMode ? 'Simulează reminderul' : 'Testează webhookul'}</button></div>` : '';
  ...
  function resetForm() { editingId = ''; $('event-form').reset(); $('event-type').value = ''; $('event-date').value = todayLocal(); $('form-title').textContent = 'Adaugă un eveniment IC'; $('save-event').textContent = 'Salvează evenimentul'; $('cancel-edit').classList.add('hidden'); }
  function fillForm(event) { editingId = String(event.id); $('event-title').value = event.title || ''; $('event-type').value = EVENT_TYPES[event.event_type] ? event.event_type : 'other'; $('event-date').value = event.event_date || todayLocal(); $('event-details').value = event.details || ''; $('event-evidence').value = event.evidence_url || ''; $('form-title').textContent = 'Editează evenimentul IC'; $('save-event').textContent = 'Salvează modificările'; $('cancel-edit').classList.remove('hidden'); window.scrollTo({ top: $('event-form-card').offsetTop - 24, behavior: 'smooth' }); }
  ...
  $('events-list').addEventListener('click', async (event) => {
    const editId = event.target.closest('[data-edit]')?.dataset.edit; const archiveId = event.target.closest('[data-archive]')?.dataset.archive; const testId = event.target.closest('[data-test]')?.dataset.test;
