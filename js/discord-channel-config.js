(() => {
 ...
 const validChannel = (value) => /^\d{15,22}$/.test(String(value || '').trim());
  ...
  const buildStashPanelPayload = () => ({ allowed_mentions: { parse: [] }, embeds: [{ title: '📦 Stash · Administrare', description: 'Gestionează articolele disponibile în Stash. Cererile și donațiile au embeduri separate.', color: 0x22c55e, fields: [{ name: 'Funcție', value: 'Adaugă și gestionează articolele din Stash.', inline: false }, { name: 'Cererile și donațiile', value: 'Sunt disponibile în embedurile separate configurate pentru aceste module.', inline: false }], footer: { text: 'Panel Pro · Stash' } }], components: [{ type: 1, components: [{ type: 2, style: 3, label: 'Adaugă în Stash', custom_id: 'panel:stash:create' }] }] });
