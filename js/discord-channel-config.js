(() => {
 ...
 const validChannel = (value) => /^\d{15,22}$/.test(String(value || '').trim());
  ...
  const buildStashPanelPayload = () => ({ allowed_mentions: { parse: [] }, embeds: [{ title: '📦 Stash · Administrare', description: 'Panou unic pentru articole, cereri și donații. Toate rezultatele sunt trimise în canalul comun de log Stash.', color: 0x22c55e, fields: [{ name: 'Funcții', value: 'Adaugă articole, solicită resurse, înregistrează donații și gestionează cererile în așteptare.', inline: false }, { name: 'Loguri', value: 'Activitatea Stash, cererile și donațiile sunt trimise împreună în canalul de log Stash.', inline: false }], footer: { text: 'Panel Pro · Stash' } }], components: [{ type: 1, components: [{ type: 2, style: 3, label: 'Adaugă în stash', custom_id: 'panel:stash:create' }, { type: 2, style: 1, label: 'Solicită articol', custom_id: 'panel:stash:request' }, { type: 2, style: 3, label: 'Donează articol', custom_id: 'panel:stash:donate' }, { type: 2, style: 1, label: 'Cereri în așteptare', custom_id: 'panel:stash:pending_requests' }, { type: 2, style: 1, label: 'Donații în așteptare', custom_id: 'panel:stash:pending_donations' }] }] });
  ...
  const buildAdditionalPanelPayload = (key) => {
    ...
    return { allowed_mentions: { parse: [] }, embeds: [{ title: definition.title, description: definition.description, color: definition.color, footer: { text: `Panel Pro · ${definition.title.replace(/^\S+\s*/, '')}` } }], components: definition.buttons.length ? [{ type: 1, components: definition.buttons.map((button) => button.style === 5 ? ({ type: 2, style: 5, label: button.label, url: button.url }) : ({ type: 2, style: button.style, label: button.label, custom_id: button.id })) }] : [] };
  };
  ...
  const syncBulkPublishState = () => {
    ...
    if (!button) return;
    button.disabled = selectedBulkDefinitions().length === 0;
  };
  const publishAllPanels = async () => {
    ...
    if (!selectedOrganizationId || !activeOrganizationId || selectedOrganizationId !== activeOrganizationId) {
      statusNode.textContent = 'Intră mai întâi în organizația aleasă din „Administrare organizații”, folosind modul de test.';
      return;
    }
    if (typeof window.sendPanelDiscord !== 'function') { statusNode.textContent = 'Modulul de trimitere Discord nu este disponibil pe această pagină.'; return; }
    ...
    if (failed.length) {
      const failedLabels = failed.map((item, index) => item.reason?.message || definitions[index]?.label).join('; ');
      statusNode.textContent = `${definitions.length - failed.length}/${definitions.length} embeduri au fost publicate. Probleme: ${failedLabels}`;
    } else {
      statusNode.textContent = `Toate cele ${definitions.length} embeduri au fost publicate sau actualizate. Canalele de rezultat/log au fost transmise și salvate împreună cu configurația.`;
    }
