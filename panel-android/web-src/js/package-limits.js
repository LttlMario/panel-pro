  ...
  const show = () => {
    ...
    const update = () => {
      help.innerHTML = select.value === 'full'
        ? '<b class="text-emerald-300">Full/Premium:</b> include Standard, partea de organizație/mafia, modulele ilegale, servere multiple, roluri nelimitate și canale Discord dedicate pentru bot.'
    ...
    actions.querySelector('#verify-package-bot-channels').onclick = () => { const routes = window.getDiscordChannelRoutes?.() || {}; const count = Object.values(routes).flatMap((route) => Object.values(route || {})).filter((item) => item?.channel_id).length; alert(count ? `Sunt selectate ${count} destinații pentru bot. Salvează organizația pentru validare.` : 'Nu există canale Discord selectate pentru bot.'); };
