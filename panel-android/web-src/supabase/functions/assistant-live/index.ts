...
/* CORS-ul este construit per request pentru rularea locală și producție. */
...
Deno.serve(async (request) => {
  const requestHeaders = getCorsHeaders(request);
  const reply = (data: unknown, status = 200) => buildReply(data, status, { ...requestHeaders, 'Access-Control-Allow-Methods': 'GET, OPTIONS', 'Cache-Control': 'no-store' });
