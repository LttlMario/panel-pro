  ...
  try {
    ...
    for (const organization of organizations || []) {
      ...
      try {
        ...
        if (!routeCandidates(settings, 'weekly_reports').some((item) => item.candidates.length)) throw new Error('Canalul Discord sau webhook-ul pentru rapoarte săptămânale nu este configurat.');
