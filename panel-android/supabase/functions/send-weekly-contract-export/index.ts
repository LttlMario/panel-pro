  ...
  try {
    ...
    for (const organization of organizations || []) {
      ...
      try {
        ...
        if (!routeCandidates(settings, 'contract_identity_weekly').some((item) => item.candidates.length)) throw new Error('Canalul Discord sau webhookul săptămânal pentru nume și CNP nu este configurat.');
