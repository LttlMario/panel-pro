...
Deno.serve(async (request) => {
  ...
  try {
    ...
    let webhookChannelIds = new Set<string>();
    let canReadWebhooks = true;
    const webhooksResponse = await fetch(`https://discord.com/api/v10/guilds/${guildId}/webhooks`, { headers: botHeaders(botToken) });
    if (webhooksResponse.ok) {
      const webhooks = await webhooksResponse.json();
      webhookChannelIds = new Set((Array.isArray(webhooks) ? webhooks : []).map((item: any) => String(item.channel_id || '')).filter(validId));
    } else if ([401, 403].includes(webhooksResponse.status)) {
    ...
    const result = rawChannels
      ...
      .map((channel: any) => ({
        ...
        has_webhook: webhookChannelIds.has(String(channel.id))
      }))
    ...
    return reply(request, { ok: true, guild: { id: guildId }, channels: result, can_read_webhooks: canReadWebhooks });
  } catch (error) {
