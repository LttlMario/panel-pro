...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(body.action==='save'){
      ...
      if(body.announcement_permissions&&typeof body.announcement_permissions==='object'){const clean=(kind:string)=>[...new Set((Array.isArray(body.announcement_permissions[kind])?body.announcement_permissions[kind]:[]).map(Number).filter((level:number)=>Number.isInteger(level)&&level>=1&&level<=99))];const {data:current}=await db.from('app_settings').select('value').eq('organization_id',organizationId).eq('key','page_permissions').maybeSingle();const value={...(current?.value||{}),announcement_permissions:{read:clean('read'),write:clean('write')}};const {error}=await db.from('app_settings').upsert({organization_id:organizationId,key:'page_permissions',value,updated_at:new Date().toISOString()},{onConflict:'organization_id,key'});if(error)throw error;}
