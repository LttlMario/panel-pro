...
if (String(body.action || '').startsWith('actions_')) {
    ...
    if (body.action === 'actions_delete') {
        ...
        const candidate = routeCandidates(settings, 'log_actions_organization', 'actions_organization').flatMap((item) => item.candidates)[0];
