...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (organizationId && !UUID_RE.test(organizationId)) {
      return reply({
        error: 'Organizația selectată este veche sau invalidă. Reîncarcă lista organizațiilor și selectează din nou.',
    ...
    if (!UUID_RE.test(configuredOrganizationId)) {
      return reply({
        error: 'Configurația organizației este invalidă. Contactează administratorul platformei.',
        code: 'ORGANIZATION_DATA_INVALID',
      }, 500);
    }
