...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!voucher || String(voucher.redeemed_by_discord_id) !== String(user.id) || String(voucher.redeemed_organization_id) !== organizationId) return reply({ error: 'Voucherul sau organizaÈ›ia nu corespunde.' }, 403);
    ...
    if (roles.some((role: any) => !/^\d{15,22}$/.test(String(role.guild_id || primaryGuildId)) || !allowedGuilds.has(String(role.guild_id || primaryGuildId)))) return reply({ error: 'Un rol selectat nu aparÈ›ine unui server configurat.' }, 400);
    ...
    if (!bot) return reply({ error: 'Botul Discord nu este configurat Ã®n Supabase.' }, 500);
    ...
    if (roles.some((role: any) => !availableRoles.has(`${String(role.guild_id || primaryGuildId)}:${String(role.id)}`))) return reply({ error: 'Un rol selectat nu existÄƒ pe server.' }, 400);
    ...
    return reply({ error: error instanceof Error ? error.message : 'Eroare internÄƒ.' }, 500);
  }
});
