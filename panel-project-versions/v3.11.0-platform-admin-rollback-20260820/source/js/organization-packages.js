(() => {
  ...
  box.innerHTML = '<h2 class="font-bold">Pachetul organizației</h2><p id="package-help" class="mt-1 text-xs text-slate-400">Pentru o organizație nouă alege pachetul și perioada. La editare pachetul existent este păstrat automat.</p><div id="package-new-fields" class="mt-3 grid md:grid-cols-2 gap-3"><label>Pachet<select id="package-code" class="field"><option value="standard">Standard</option><option value="full">Full</option></select></label><label>Activ p�nă la<input id="package-expires" type="datetime-local" class="field"></label></div><div id="package-existing-fields" class="mt-3 hidden"><p id="package-current" class="text-sm font-bold text-fuchsia-200"></p><div class="mt-2 flex flex-wrap gap-2"><button type="button" data-package-days="7" class="rounded-lg border border-fuchsia-700 px-3 py-2 text-xs font-bold">+ 7 zile</button><button type="button" data-package-days="30" class="rounded-lg border border-fuchsia-700 px-3 py-2 text-xs font-bold">+ 30 zile</button><button type="button" data-package-days="-7" class="rounded-lg border border-rose-700 px-3 py-2 text-xs font-bold">− 7 zile</button></div></div><p id="package-status" class="mt-3 text-xs text-slate-400"></p>';
  ...
  const syncMode = () => {
    ...
    box.querySelector('#package-current').textContent = `Pachet: ${packageValue.code || 'existent'} · ${packageValue.unlimited ? 'fără termen' : (packageValue.expires_at ? `p�nă la ${new Date(packageValue.expires_at).toLocaleString('ro-RO')}` : 'termen gestionat separat')}`;
  };
