(() => {
  ...
  const render = (items) => {
    list.innerHTML = items.map((item) => `<article class="secret-card" data-secret-card="${escapeHtml(item.name)}">
      ...
      <div class="mt-4 flex flex-wrap gap-2"><button type="button" class="secret-button secondary" data-change>Schimbă valoarea</button><button type="button" class="secret-button primary" data-apply>Aplică peste tot</button>${String(item.name).startsWith('public_community_channel_') || String(item.name).startsWith('public_rating_channel_') ? '<button type="button" class="secret-button secondary" data-test>Testează canalul botului</button>' : ''}</div>
    </article>`).join('');
    list.querySelectorAll('[data-secret-card]').forEach((card) => {
      ...
      change.addEventListener('click', async () => {
        if (editor.classList.contains('hidden')) { editor.classList.remove('hidden'); input.focus(); change.textContent = 'Salvează valoarea'; return; }
        ...
        try { const result = await request({ action:'set', name, value:input.value }); input.value=''; editor.classList.add('hidden'); change.textContent='Schimbă valoarea'; setStatus(`Valoarea a fost salvată și aplicată în ${(result.applied_to || []).length} locuri.`, 'success'); await load(); } catch (error) { setStatus(error.message, 'error'); } finally { change.disabled=false; apply.disabled=false; }
      });
      ...
      test?.addEventListener('click', async () => { test.disabled=true; setStatus('Se testează canalul botului…'); try { await request({action:'test_bot_channel',name}); setStatus('Botul a postat cu succes în canal.', 'success'); } catch(error) { setStatus(error.message, 'error'); } finally { test.disabled=false; } });
    });
  };
